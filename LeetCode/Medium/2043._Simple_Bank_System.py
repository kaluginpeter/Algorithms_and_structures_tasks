...
# Solution
# C++ O(N) for init and O(1) for query O(N) Memory Array
class Bank {
private:
...
# Python O(N) for init and O(1) for each query. Memory O(N) Array
class Bank:
    ...
    def is_enough_balance(self, account: int, bound: int) -> bool:
        return self.balance[account - 1] >= bound

    def transfer(self, account1: int, account2: int, money: int) -> bool:
        if not self.is_valid_account(account1) or not self.is_valid_account(account2):
            return False
        if not self.is_enough_balance(account1, money): return False
        self.balance[account1 - 1] -= money
        self.balance[account2 - 1] += money
        return True

    def deposit(self, account: int, money: int) -> bool:
        if not self.is_valid_account(account): return False
        self.balance[account - 1] += money
        return True

    def withdraw(self, account: int, money: int) -> bool:
        if not self.is_valid_account(account): return False
        if not self.is_enough_balance(account, money): return False
        self.balance[account - 1] -= money
        return True
