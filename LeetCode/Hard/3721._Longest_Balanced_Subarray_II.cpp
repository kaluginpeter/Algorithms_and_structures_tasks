...
*/
// Solution
// C++ O(NlogN) O(N) SegmentTree PrefixSum
struct LazyTag {
    int to_add = 0;
...
private:
    ...
    int find(int target_l, int target_r, int val, int l, int r, int i) {
        if (tree[i].min_value > val || tree[i].max_value < val) return -1;
        if (l == r) return l;
        pushdown(i);
        int mid = l + ((r - l) >> 1);
        if (target_r >= mid + 1) {
        ...
        if (l <= target_r && mid >= target_l) return find(target_l, target_r, val, l, mid, i << 1);
        return -1;
    }
};
...
public:
    int longestBalanced(vector<int>& nums) {
        ...
        for (int i = 1; i < nums.size(); ++i) {
            ...
            if (occ.empty()) prefix_sum[i] += sgn(nums[i]);
            occ.push(i + 1);
        }
        ...
        for (int i = 0; i < nums.size(); ++i) {
            ...
            if (!occurrences[nums[i]].empty()) next_pos = occurrences[nums[i]].front();
