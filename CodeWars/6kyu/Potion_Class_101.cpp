...
class Potion {
  public:
    ...
    Potion mix(const Potion& other) {
      unsigned int newVolume = this->volume + other.volume;
      std::array<unsigned char, 3> newColor {};
      for (int i = 0; i < 3; i++) {
        double weighted = (this->color[i] * this->volume + other.color[i] * other.volume)
                              / static_cast<double>(newVolume);
        newColor[i] = static_cast<int>(std::ceil(weighted));
        }
      return Potion(newColor, newVolume);
    };
};