# In Spanish, the conjugated verb changes by adding suffixes and according to the
# person we're talking about. There's something similar in English when we
# talk about "She", "He"or "It" (3rd person singular):
# With the verb "run":
# He / She / It runS
# As you can see, the rule (at least with regular verbs) is to add the
# suffix "-s" in the 3rd person singular. In Spanish it works the same way
# but we need to remove the infinitive suffix and add a specific suffix to all
# the others persons (I, You, He/She/It, We, You, They).
# Verbs in Spanish and the infinitive suffix.
# In Spanish we assume a verb is on its infitive form when it has
# one of the infinitives suffixes (AR, ER or IR) at the end:
# Comer -> to eat
# Caminar -> to walk
# Vivir -> to live
# How to conjugate
# For conjugating in Spanish, we need to remove the infinitive suffix (ar, er or ir)
# and add the personal suffixes corresponding to the person we're talking to.
# In this kata we'll conjugate the verbs to its presente indicativo (simple present) form.
# Personal suffixes
...
# Viven (They live)
# Your Task
# You need to write a function called conjugate which will return an object with a
# spanish verb conjugated. The object must look like this:
#
# {
#   "comer": [
#     "como",
...
#   ]
# }
# Where the key is the verb in its original form (infinitive form) and its value
# will be an array with the conjugations.
#
# Another example:
#
# {
