...
#
# Given 4 integers a, b, c, d we form the sum of the squares of a and b and then the sum of the squares
# of c and d. We multiply the two sums hence a number n and we try to decompose n in a sum of two
# squares e and f (e and f integers >= 0) so that n = e² + f².
#
# More: e and f must result only from sums (or differences) of products between on the one hand (a, b)
# and on the other (c, d) each of a, b, c, d taken only once. For example, prod2sum(1, 2, 1, 3) should
# return [[1, 7], [5, 5]]) because
#
# 1==1*3-1*2
# 7==2*3+1*1
# 5==1*2+1*3
...
#
# The return of our function will be an array of subarrays (in C an array of Pairs) sorted
# on the first elements of the subarrays. In each subarray the lower element should be the first.
#
