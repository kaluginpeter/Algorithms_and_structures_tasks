...
class GuessWho():
    ...
    def __init__(self, character):
        ...
        self.secret = character
        self.turns = 0
        self.possible = self.characteristic[:]

        self.lookup = {
            name: set(attrs)
            for name, attrs in self.characteristic
        }

    def guess(self, guess):
        self.turns += 1
        if guess == self.secret: return [f"Correct! in {self.turns} turns"]
        if guess in self.lookup:
            self.possible = [
                item for item in self.possible
                if item[0] != guess
            ]
        else:
            has_trait = guess in self.lookup[self.secret]
            if has_trait:
                self.possible = [
                    item for item in self.possible
                    if guess in item[1]
                ...
                self.possible = [
                    item for item in self.possible
                    if guess not in item[1]
                ]

        return [name for name, _ in self.possible]